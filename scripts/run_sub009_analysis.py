"""
Run complete validation pipeline for sub-009.

Protocol: 18 trials (6 per condition: LEFT, RIGHT, NOTHING), 15s task, 15s rest.
...
def main() -> int:
    """Run the complete pipeline for sub-009."""
    ...
    # Load optimized configuration for sub-009
    config_path = Path("configs/sub009_optimized.yml")
    if config_path.exists():
        print(f"Loading configuration from: {config_path}")
        config = PipelineConfig.from_yaml(config_path)
    else:
        print("WARNING: sub009_optimized.yml not found, using default configuration")
        config = PipelineConfig.default()
    
    print("=" * 70)
    print("Running complete validation pipeline for sub-009")
    print("Protocol: 18 trials (6 per condition), 15s task, 15s rest")
