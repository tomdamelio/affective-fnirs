"""
Run complete validation pipeline for sub-003.

This script executes the full multimodal EEG + fNIRS validation pipeline
for sub-003 with shorter trial parameters (3 trials, 3s duration, 3s rest).
"""
...
def main():
    """Run the complete pipeline for sub-003."""
    # Paths
    xdf_file = Path("data/raw/sub-003/sub-003_ses-001_task-fingertapping_recording.xdf")
    eeg_json = Path("data/raw/sub-003/sub-003_ses-001_task-fingertapping_eeg.json")
    fnirs_json = Path("data/raw/sub-003/sub-003_ses-001_task-fingertapping_nirs.json")
    output_dir = Path("data/derivatives/validation-pipeline/sub-003/ses-001")

    # Load optimized configuration for sub-003
    config_path = Path("configs/sub003_optimized.yml")
    if config_path.exists():
        print(f"Loading optimized configuration from: {config_path}")
        config = PipelineConfig.from_yaml(config_path)
    else:
        print("WARNING: sub003_optimized.yml not found, using default configuration")
        config = PipelineConfig.default()
    
    print("=" * 70)
    print("Running complete validation pipeline for sub-003")
    print(f"Trial parameters: 3 trials, 3s duration, 3s rest")
    ...
    if not np.isnan(results.erd_metrics.beta_rebound_percent):
        print(f"  Beta Rebound (3.5-5s): {results.erd_metrics.beta_rebound_percent:.2f}%")
    else:
    ...
    if results.erd_metrics_c4 is not None:
        ...
        if not np.isnan(results.erd_metrics_c4.beta_rebound_percent):
            print(f"  Beta Rebound (3.5-5s): {results.erd_metrics_c4.beta_rebound_percent:.2f}%")
        else:
            print(f"  Beta Rebound: N/A (check epoch range)")
