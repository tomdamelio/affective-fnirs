"""
Run QA-only report for sub-005.
...
def main() -> int:
    """Run QA-only pipeline for sub-005."""
    ...
    # Get channel counts from data shape
    ...
    summary = {
        ...
        "fnirs": {
            ...
            "n_channels": fnirs_n_channels,
            "sample_rate_hz": round(fnirs_srate, 2),
            "n_samples": int(fnirs_data.shape[1])
        },
        "markers": {
            "total": len(markers),
    ...
    summary_path = output_dir / "sub-005_ses-001_task-fingertapping_desc-qa_summary.json"
