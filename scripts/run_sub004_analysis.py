"""
Run complete validation pipeline for sub-004.
...
def main():
    """Run the complete pipeline for sub-004."""
    ...
    # Load optimized configuration for sub-004
    config_path = Path("configs/sub004_optimized.yml")
    if config_path.exists():
        print(f"Loading optimized configuration from: {config_path}")
        config = PipelineConfig.from_yaml(config_path)
    else:
        print("WARNING: sub004_optimized.yml not found, using default configuration")
        config = PipelineConfig.default()
    
    print("=" * 70)
    print("Running complete validation pipeline for sub-004")
