"""
Run complete validation pipeline for sub-010.

Protocol: 36 trials (12 per condition: LEFT, RIGHT, NOTHING), 15s task, 15s rest.
EEG: ~30 channels (10-20 system, missing CP5, CP6), CAR reference.
"""
...
def main() -> int:
    """Run the complete pipeline for sub-010."""
    ...
    fnirs_json = Path("data/raw/sub-010/sub-010_ses-001_task-fingertapping_nirs.json")
    ...
    # Load optimized configuration for sub-010
    config_path = Path("configs/sub010_optimized.yml")
    if config_path.exists():
        print(f"Loading configuration from: {config_path}")
        config = PipelineConfig.from_yaml(config_path)
    else:
        print("WARNING: sub010_optimized.yml not found, using default configuration")
        config = PipelineConfig.default()
    
    print("=" * 70)
    print("Running complete validation pipeline for sub-010")
    print("Protocol: 36 trials (12 per condition), 15s task, 15s rest")
    print("EEG: ~30 channels (10-20 system, missing CP5, CP6)")
    ...
    if results.erd_metrics_c4 is not None:
        ...
        if not np.isnan(results.erd_metrics_c4.beta_rebound_percent):
            print(f"  Beta Rebound: {results.erd_metrics_c4.beta_rebound_percent:.2f}%")
        else:
            print(f"  Beta Rebound: N/A")
